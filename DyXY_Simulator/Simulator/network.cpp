...
network_t::network_t(int p) {
  ...
  for (int p = 0; p < NUM_PRIORITIES; ++p) {
    ...
    for (int i = 0; i < num_ports; ++i) {
      ...
      if(p == PRI0){
        from_net_fifos[p][i]->init_time(name, 1024*4, empty);
      }
      else{
        from_net_fifos[p][i]->init_time(name, 1024*4, empty);
      }
...
bool network_t::to_net(int src_port, pri_t pri, net_cmd_t cmd) {
  ...
  int latency = cur_cycle + 1 + CONTROL_LATENCY;
  if (space_p){
    if(pri != PRI0){ latency += NETWORK_LATENCY - 1 - CONTROL_LATENCY;}
    from_net_fifos[pri][cmd.dest]->tenqueue(latency, cmd);
  }
