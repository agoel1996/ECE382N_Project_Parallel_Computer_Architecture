...
int cur_cycle = 0;
int count_pkt[MESH_DIM*MESH_DIM][MESH_DIM*MESH_DIM];
int latency[MESH_DIM*MESH_DIM][MESH_DIM*MESH_DIM];
int shortestPath[MESH_DIM*MESH_DIM][MESH_DIM*MESH_DIM];
int CONTROL_LATENCY = 0;
int NUM_SRC = 0;
...
void parse_args(int argc, char *argv[]) {
  if (argc != 9) {
    ERROR("usage: <number of processors> <num cycles> <num packets> <control latency> <test> <verbose>");
  }
  
  args.num_procs = atoi(argv[1]);
  args.num_cycles = atoi(argv[2]);
  args.numSrc = atoi(argv[3]);
  args.numPkts = atoi(argv[4]); 
  args.cntrLatency = atoi(argv[5]);
  args.test = atoi(argv[6]);
  args.verbose = atoi(argv[7]);
  srand(atoi(argv[8]));

  CONTROL_LATENCY = args.cntrLatency;
  NUM_SRC = args.numSrc;
...
void init_system() {
  ...
  for (int p = 0; p < args.num_procs; ++p) {
    ...
    ius[p]->routingTableInit();
    for(int dest = 0; dest < MESH_DIM*MESH_DIM; dest++) {
      int src_x = p%MESH_DIM;
      int src_y = p/MESH_DIM;
      int dest_x = dest%MESH_DIM;
      int dest_y = dest/MESH_DIM;
      int sPath = abs(dest_x-src_x) + abs(dest_y-src_y);
      shortestPath[p][dest] = sPath*NETWORK_LATENCY; 
      //if(p == 0 && dest == 57)
      //  printf("srcx = %d, srcy =%d, destx = %d, desty=%d, spath=%d,shortP=%d\n", src_x,src_y,dest_x,dest_y,sPath, shortestPath[p][dest]);
    }
  }
